...
/* DPDMA overlay frame buffer (defined in xdpdma_video_example.c) */
extern u8 Frame[];
...
int main()
{
	...
	/* Disable VTC and freeze register updates before reconfiguring.
	 * XVtc_Reset() polls for completion and hangs if the pixel clock is not
	 * yet running (e.g. immediately after a board button reset). */
	XVtc_Disable(&VtcInst);
	XVtc_RegUpdateDisable(&VtcInst);
	/* Configure timing and source into shadow registers (not yet applied) */
	XVtc_SetGeneratorTiming(&VtcInst, &vtcTiming);
	XVtc_SetSource(&VtcInst, &SourceSelect);
	XVtc_EnableGenerator(&VtcInst);
	/* Atomically propagate the configured values, then enable */
	XVtc_RegUpdateEnable(&VtcInst);
	XVtc_Enable(&VtcInst);
#endif /* !COLOR_TEST_ONLY */
	...
	vdmaConfig = XAxiVdma_LookupConfig(XPAR_XAXIVDMA_0_BASEADDR);
...
#ifdef COLOR_TEST_ONLY
	{
		/* Cycle through solid colors in the DPDMA overlay.
		 * RGBA8888 pixel layout (little-endian u32 = 0xAABBGGRR):
		...
		const char *names[] = {"Red", "Green", "Blue", "White", "Cyan", "Magenta", "Yellow"};
		u32 *bufs[2] = { (u32 *)Frame, (u32 *)Frame2 };
		int back = 1;   // Frame is initial display (front), Frame2 is first back buffer
		int idx = 0;
		while(1) {
			xil_printf("Display color: %s\n\r", names[idx]);
			u32 *bb = bufs[back];
			for (int p = 0; p < DEMO_MAX_FRAME; p++) bb[p] = colors[idx];
			flip_frame_buffer((u8 *)bb);  // DPDMA swaps at next vsync
			back ^= 1;
			idx = (idx + 1) % 7;
			usleep(2000000);
		}
